...
class LSU extends Module {
  val io = IO(new Bundle {
    ...
    //    val lsu_busy: Bool = Input(Bool()) // LSU 正在等待内存传回数据
    ...
    //    val data_r_o: UInt = Input(UInt(32.W)) // 内存返回给 LSU 的数据
  ...
  data_mem.io.addr := io.addr_i
  data_mem.io.data_w_req := io.write_enable
  data_mem.io.data_w := io.data_w_i

  data_mem.io.data_r_req := io.read_req
  io.data_r_o := data_mem.io.data_r
  io.data_vaild := data_mem.io.read_data_vaild
